  ...
  addProductInCart = async (product) => {
    try {
      ...
      if (productInCart) {
        ...
        getCarts[cartPositionInArray].products[productPositionInArray] = {
          ...updatedProduct,
        };
      ...
      getCarts[cartPositionInArray].products = [
        ...getCarts[cartPositionInArray].products,
        productAdd,
      ];
  ...
  getCartById = async (cid) => {
    try {
      ...
      if (cartsInJson.length > 0) {
        ...
        const cartPositionInArray = cartsInJson.findIndex(
          (cart) => +cart.id === +cid
        );
  ...
  getProductById = async (cid, pid) => {
    try {
      ...
      if (findCart) {
        ...
        const productPositionInArray = findCart.products.findIndex(
